  ...
  return (
    <main className="flex-1 w-full max-w-[820px] mx-auto p-6 md:p-10 mb-20 mt-4 md:mt-8 bg-surface rounded-3xl shadow-sm border border-border">
      <section className="text-center">
        <p className="mb-3 text-sm font-bold text-primary">404</p>
      ...
      <nav aria-label="도움이 되는 링크" className="mt-10 grid gap-3 sm:grid-cols-2">
        {helpfulLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="rounded-2xl border border-border bg-background px-5 py-4 text-sm font-bold text-foreground hover:text-primary transition-colors"
          >
            {link.label}
          </Link>
        ))}
