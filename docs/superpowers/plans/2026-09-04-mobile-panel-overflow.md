...
- Next.js 16.2.4의 `turbopack.root`는 `__dirname`으로 고정해 중첩 worktree가 자신의 프로젝트 소스를 서빙하게 한다.
...
- Produces: 768px 이상에서 부모 높이를 넘지 않고 내부 스크롤하는 비교 패널, 상호작용 손잡이가 없는 정적 `BottomSheet` 표면, 중첩 worktree에서도 현재 프로젝트를 서빙하는 Turbopack root.

- [x] **Step 1: 푸터 경계 회귀 E2E 테스트 작성**
...
`next.config.ts`에는 Next.js 16.2.4의 `turbopack.root: __dirname`을 설정해, lockfile 자동 감지가 바깥 저장소를 선택하지 않고 중첩 worktree가 자신의 프로젝트를 서빙하게 한다.
...
예상 결과: 모든 명령이 종료 코드 0을 반환한다. `turbopack.root` 수정 뒤 깨끗한 서버에서 전체 E2E를 실행하고, build 뒤에도 전체 E2E를 다시 실행해 중첩 worktree root 혼동이 재발하지 않는지 확인한다. 갱신된 스냅샷은 손잡이 제거와 패널 경계 수정 외의 의도치 않은 변화가 없어야 한다.

- [x] **Step 7: 작은 커밋 작성**

계획, 패널 구현, Turbopack root 수정을 각각 정확한 파일만 스테이징해 작은 커밋으로 남긴다.

```bash
git commit -m "docs: 모바일 패널 수정 계획 추가"
git commit -m "fix: 푸터 경계와 고정 비교 패널 정정"
git commit -m "fix: Turbopack 작업공간 루트 고정"
```
